    ...
    <div class="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6">
        ...
        <div class="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700 grid grid-cols-2 gap-3 text-sm">
            <div>
                ...
                <div class="font-medium">{{ \App\Models\AttendanceSession::TYPES[$record->type] ?? $record->type }}</div>
            </div>
            <div>
                <div class="text-xs text-gray-500 dark:text-gray-400">Total checked-in</div>
                <div class="font-semibold text-primary-600">{{ $roster->count() }}</div>
            </div>
            @if ($record->starts_at)
                <div>
                    <div class="text-xs text-gray-500 dark:text-gray-400">Opens</div>
                    <div>{{ $record->starts_at->format('M d, H:i') }}</div>
                </div>
            @endif
            @if ($record->ends_at)
                <div>
                    <div class="text-xs text-gray-500 dark:text-gray-400">Closes</div>
                    <div>{{ $record->ends_at->format('M d, H:i') }}</div>
                </div>
            @endif
        </div>
    </div>
...
<script>
    (function () {
        // Wait for the QR div to actually be in the DOM (Filament modal can mount async)
        function tryRenderQr() {
            var el = document.getElementById('attendance-qr');
            if (!el) return false;
            if (el.dataset.rendered === 'yes') return true;
        ...
        if (!tryRenderQr()) {
            ...
            var iv = setInterval(function () {
                attempts++;
                if (tryRenderQr() || attempts > 20) clearInterval(iv);
            }, 150);
        }
    })();
</script>
